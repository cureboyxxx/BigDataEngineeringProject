...
- Used in: `notebooks/04-amazon-electronics-dataset-cleaning.ipynb`
...
- Produced by: `notebooks/01-frankfurter-exchange-rate-API-request.ipynb`
- Used in: `notebooks/04-amazon-electronics-dataset-cleaning.ipynb`
...
- Produced by: `notebooks/03-iphone-models-webscraping.ipynb`
- Used in: `notebooks/05-analysis-iPhones-Amazon-value-vs-news-attention.ipynb`
...
- Produced by: `notebooks/02-gdelt-iphone-news-mentions-API-request.ipynb`
- Used in: `notebooks/05-analysis-iPhones-Amazon-value-vs-news-attention.ipynb`
...
- Produced by: `notebooks/04-amazon-electronics-dataset-cleaning.ipynb`
- Used in: `notebooks/05-analysis-iPhones-Amazon-value-vs-news-attention.ipynb`
...
```mermaid
flowchart TD
    frankfurter_api["(2) Frankfurter API (INR to EUR exchange rates)"]
    gdelt_api["(4) GDELT API (2023 iPhone news mentions)"]
    wikipedia_page["(3) Wikipedia iPhone page (iPhone model metadata)"]
    kaggle["(1) Kaggle Amazon Electronics Dataset"]
    amazon_raw["data/All Electronics.csv>"]

    nb01["01-frankfurter-exchange-rate-API-request.ipynb"]
    nb02["02-gdelt-iphone-news-mentions-API-request.ipynb"]
    nb03["03-iphone-models-webscraping.ipynb"]
    nb04["04-amazon-electronics-dataset-cleaning.ipynb"]
    nb05["05-analysis-iPhones-Amazon-value-vs-news-attention.ipynb"]
    ...
    result["iPhone value vs news-attention analysis"]
...
1. `01-frankfurter-exchange-rate-API-request.ipynb` requests 2023 INR to EUR exchange rates from Frankfurter and writes `data/frankfurter-inr-eur-exchange-rates-2023.csv`.

2. `02-gdelt-iphone-news-mentions-API-request.ipynb` requests 2023 GDELT news mention counts for the selected iPhone models and writes `data/gdelt-iphone-news-mentions-2023.csv`.

3. `03-iphone-models-webscraping.ipynb` scrapes iPhone model metadata from Wikipedia, normalizes model names, and writes `data/wikipedia-iphone-models.csv`.

4. `04-amazon-electronics-dataset-cleaning.ipynb` reads the raw Amazon dataset from `data/All Electronics.csv` and the exchange-rate data from `data/frankfurter-inr-eur-exchange-rates-2023.csv`. It cleans the product data, converts prices to EUR, validates the result, and writes `data/cleaned-amazon-electronics-sales-2023.csv`.

5. `05-analysis-iPhones-Amazon-value-vs-news-attention.ipynb` reads:
